...
def get_leagues_id_line() -> list:
    ...
    try:
        response = requests.get('https://melbet.ru/LineFeed/GetSportsZip', params=params, cookies=cookies, headers=headers)
        # Операции с сохранением json
        with open("line/get_sport_zip_line_json.json", "w") as file:
            file.write(response.text)
        with open("line/get_sport_zip_line_json.json") as f:
            temp = json.load(f)
        with open('line/get_sport_zip_line_json.json', 'w') as f:
            json.dump(temp, f, sort_keys=True, indent=20, ensure_ascii=False)
        # Достаем информацию из json
        with open('line/get_sport_zip_line_json.json') as file:
